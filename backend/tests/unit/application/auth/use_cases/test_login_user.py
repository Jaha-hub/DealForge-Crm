...
from tests.unit.application.auth.interfaces.security.fake_token import FakeToken
...
from tests.unit.domain.user.value_objects.test_entity import valid_user, password_hash, user_id
...
@pytest.mark.asyncio
async def test_login_user_raises_user_not_found(login_user_command: LoginUserCommand):
...
@pytest.mark.asyncio
async def test_login_user_raises_invalid_password(login_user_command: LoginUserCommand,valid_user: User):
...
@pytest.mark.asyncio
async def test_login_user_raises_inactive_user(login_user_command: LoginUserCommand,valid_user: User):
