...
from src.backend.domain.source.enums.value_objects.source_config.errors import EmptyWebhookSecretError, \
    InvalidFieldMappingError, InvalidSlugError, EmptyFormFieldError, DuplicateFormFieldError
...
class PublicFormConfig:
    ...
    def __post_init__(self):
        if not self.slug or not (1 <= len(self.slug) <= 100):
